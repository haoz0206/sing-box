...
- Textual 引导式 TUI：Dashboard 只保留服务状态、配置计数与唯一安全建议，完整配置
  清单和生命周期入口集中在独立 Profiles 工作区，可通过 `p` 或可见按钮进入；后台服务健康检查、可操作诊断、协议引导、
  上下文键盘快捷键与 `?` 操作帮助、能力感知的运维中心（集中进入核心计划、
...
- 配置详情中的引导式编辑：可修正名称、公开服务器地址和监听端口；端口支持固定值
  ...
  完整校验、刷新、健康检查和回滚的 live configuration 变更；
...
- 版本化 desired state、原子 JSON 保存、修订冲突保护和上一版备份；主文件损坏时
  TUI 可预览备份 revision/配置数量，经二次确认和双 SHA-256 复检后原子恢复，
  同时保留损坏原字节；未来 schema 不会被降级覆盖；
