...
[![CI](https://github.com/haoz0206/sing-box/actions/workflows/ci.yml/badge.svg)](https://github.com/haoz0206/sing-box/actions/workflows/ci.yml)
[![Security](https://github.com/haoz0206/sing-box/actions/workflows/security.yml/badge.svg)](https://github.com/haoz0206/sing-box/actions/workflows/security.yml)
[![License: GPL-3.0](https://img.shields.io/badge/License-GPL--3.0-blue.svg)](LICENSE)

一个以安全计划、显式确认和可恢复变更为核心的 sing-box 管理 TUI。

本项目源于 233boy/sing-box 管理脚本，目前正在独立演进为 Python 应用。这是新的产品
设计，不以复刻旧脚本行为为目标。旧 Bash 文件暂时保留作迁移与功能覆盖参考，不会被
Python 入口调用。
...
- Textual 引导式 TUI：Dashboard 只保留服务状态、配置计数与唯一安全建议，完整配置
  ...
  配置详情使用可滚动只读视图展示身份、状态、服务器地址和端口意图，含凭据的客户端
  payload（分享 URI 或 Surge 策略）默认隐藏且仅能显式显示一次；详情与披露错误文案
  统一由目录渲染，生命周期按钮只
...
- 编辑、移除、暂停/恢复、模板创建或 desired-state 恢复成功返回首页时，会统一清除
  ...
  停留在“正在检查”或短暂沿用变更前结论；
- 包含完整访问凭据的客户端 payload（分享 URI 或 Surge 策略）在持久化配置详情和
  首次应用成功页均默认隐藏；页面只展示公开端点与风险说明，必须由操作者在私密
  终端中显式选择“显示一次”才会挂载只读文本；操作者可立即重新隐藏且本页不能二次
  显示，离开页面后也会自动恢复隐藏，整个过程不会自动写入剪贴板；
...
- VLESS Reality、VLESS/VMess TLS WebSocket/gRPC、Shadowsocks 2022、Hysteria2、Trojan、AnyTLS、TUIC，以及版本受限 Snell v6 的引导、凭据生成、类型化客户端结果和多 profile 配置；
...
Snell 以 active 核心实际报告的精确版本判断能力，而不是依据 Stable/Preview 标签。
当前仅支持 sing-box `1.14.0-alpha.38` 或更新版本上的 v6/default mode/单一顶层 PSK；
Stable 1.13.x 与未知核心会在 Snell 规划前保守停止。请先激活满足最低版本的 Preview，
再重新生成计划。已应用且启用的 Snell 会阻止切换到不兼容核心；草案或暂停状态不会，
因此暂停或移除最后一个 active Snell 是回到 Stable 的恢复路径。

客户端结果是 Surge 策略 `Name = snell, host, port, psk=..., version=6`，不是
`snell://` URI；策略名可在 Surge 中修改。完整 payload 含凭据，默认隐藏，必须由
操作者显式显示一次。范围和运维步骤见[软件使用手册](docs/MANUAL.md)。
...
- 提交问题或代码前请阅读 [贡献指南](CONTRIBUTING.md) 与 [行为准则](CODE_OF_CONDUCT.md)。
- 安全问题请按 [安全策略](SECURITY.md) 私下报告，不要公开披露凭据或主机信息。
- 用户可见变更记录在 [CHANGELOG](CHANGELOG.md)，维护者发布步骤见
  [发布指南](RELEASING.md)。
- 版本遵循语义化版本；带注释的 `vX.Y.Z` 标签通过受保护的 GitHub Actions 工作流生成
  wheel、sdist、SHA-256 校验和与构建来源证明。
...
- [精确核心版本与协议能力决策](docs/adr/0025-core-version-protocol-capabilities.md)
...
- [2026-07-18 Snell v6 Preview 支持验收](docs/acceptance/2026-07-18-snell-v6-preview-support.md)
