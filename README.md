...
- 按需加载的诊断中心：统一检查 desired state 一致性、配置目标、最小权限
  helper、核心与 runtime，按“需处理/注意/正常”排序并给出唯一推荐动作；
- 配置详情中的生命周期移除：草案只提交 desired state；已应用配置先预览影响，
  再生成完整剩余配置并复用校验、原子提交、服务健康检查和自动回滚；
- 配置详情中的引导式编辑：可修正名称和公开服务器地址，预览会区分仅 desired
  state 的元数据变更与需要校验、刷新、健康检查和回滚的 live configuration 变更；
...
任何协议扩展都应作为完整的 UI-to-config 垂直切片交付，而不是把协议判断继续堆进 UI 或系统脚本。创建、应用和移除使用同一个完整配置投影规则，避免不同生命周期操作生成不一致的 sing-box 文档。
