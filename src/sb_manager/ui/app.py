...
class CoreUpdatePlanScreen(Screen[None]):
    ...
    def execute_core_update(self) -> None:
        try:
            ...
            self.app.call_from_thread(
                self.app.push_screen,
                CoreUpdateErrorScreen(str(error), host_result_unknown=False),
            )
            return
        except CoreActivationError as error:
            self.app.call_from_thread(
                self.app.push_screen,
                CoreUpdateErrorScreen(str(error), host_result_unknown=True),
            )
            return
        self.app.call_from_thread(self.app.push_screen, CoreUpdateResultScreen(result))


class CoreUpdateFormScreen(Screen[None]):
    """Collect an exact version and explicit prerelease policy."""
    ...
    def preview_core_update(self) -> None:
        ...
        try:
            ...
            return
        self.app.push_screen(CoreUpdatePlanScreen(self.core_updater, plan))
